...
match_cols <-  c("cohort", "collect_year_cat", "collect_age_cat", "menopause_stt")
...
fct_cols <- c("gp", "cohort", "collect_age_cat", "collect_year_cat",
              "menopause_stt",
              "ethnicity",
              "edu_level",
              "income_level",
...
int_cols <- c("sdc_age_calc", "wh_gravidity", "wh_live_births", "collect_year")
...
import_survey <- function(){
  full_dat <- read_csv("C:/Users/lyhtr/OneDrive - UBC/Thesis/Data/exposome/data_with_missing.csv") %>%
...
import_meta_sc <- function(){
  full_sc <- read_csv("C:/Users/lyhtr/OneDrive - UBC/Thesis/Data//exposome/normalized_metabolomics_scaled.csv")
}
...
import_imp <- function() {
  ## Impute data
  path <- Sys.glob("C:/Users/lyhtr/OneDrive - UBC/Thesis/Data/exposome/Bootstrapped_data/*.csv")
