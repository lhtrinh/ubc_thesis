...
dup_cols <- temp1 %>% select(starts_with("f1_")) %>% names()
length(dup_cols)
# 42 columns from follow-up questionnaire

dedup_cols <- str_replace_all(dup_cols, "f1_", "")
dat_cols <- temp1[,names(temp1) %in% dedup_cols] %>% names()
length(dat_cols)
...
dups1 <- data.frame(
  og = dedup_cols[which(dedup_cols %in% dat_cols)],
  dup = dup_cols[which(dedup_cols %in% dat_cols)])
